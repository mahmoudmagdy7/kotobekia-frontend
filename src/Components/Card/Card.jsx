    ...
    <>
      <div className=" m-auto h-full pt-1 mb-5  ">
        <div className=" mx-2   shadow-[0_11px_22.5px_-15px_rgba(0,0,0,0.1)] rounded-xl">
          <figure className="w-full h-48 overflow-hidden rounded-t-xl relative">
            <img className="w-full object-fill " src={"https://kotobekia-backend.onrender.com/" + item?.images[0]} alt="img" />
          </figure>{" "}
          <div className="txt bg-white py-2 px-3 rounded-b-[14px]">
            <div className="card-title flex items-center justify-between gap-2">
              <h3 className=" text-[14px] md:text-[16px] text-black font-semibold  line-clamp-1">{item?.title}</h3>

              <span className="  py-[2px] px-[10px] text-[12px] bg-[#d4f7ef] text-[#38BC9C]  rounded-[8px]">{item?.price}</span>
            </div>

            <div className="cardDesc my-3">
              <p className="m-0 text-[10px] md:text-sm text-[#464646]   line-clamp-2 ">{item?.description} </p>
            </div>
            ...
            <div className="cardVarient text-[#0F172A] my-1 pb-1 border-b-1 border-[#EFEFEF] flex items-center justify-between flex-wrap">
              <h5 className="md:text-[14px] text-xs">
                {t(`grades.${item?.grade}`)}, {config.getCategoryName(item?.educationLevel)}
              </h5>
              <div className="icons flex gap-2 items-center mt-2">
                <div className="item flex items-center gap-1">
                  <span className="text-[#0F172A] text-[14px] ">{item?.numberOfBooks}</span>
                ...
                <div className="item flex items-center gap-1">
                  <span className="text-[#0F172A] text-[14px]">{item?.views}</span>
                  <span>
            ...
            <div className="cardAddress flex items-center justify-between sm:text-[14px] text-xs">
              <div className="address flex gap-1 items-center ">
                ...
                <span className="text-[#464646] text ">{item?.location}</span>
              </div>
